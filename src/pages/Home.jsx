        ...
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center min-h-[65vh]">
          <motion.div initial={{ opacity: 0, x: -40 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.9 }}>
            <motion.div initial={{ scaleX: 0 }} animate={{ scaleX: 1 }} transition={{ duration: 0.8 }}
              className="w-14 h-[3px] bg-orange-500 mb-6 origin-left" />
            <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }}
              className="text-orange-600 text-xs font-semibold uppercase tracking-[0.3em] mb-6" style={{ fontFamily: 'var(--font-sans)' }}>
              {hero.badge}
            </motion.p>
            <div className="overflow-hidden">
              <motion.h1 initial={{ y: '100%' }} animate={{ y: 0 }} transition={{ duration: 1, delay: 0.5, ease: [0.16, 1, 0.3, 1] }}
                ...
                TOP BRANDS.<br /><span className="text-orange-600 italic font-bold">BEST PRICES.</span>
              </motion.h1>
            </div>
            <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1 }}
              className="text-gray-500 text-sm leading-relaxed mt-6 max-w-lg" style={{ fontFamily: 'var(--font-sans)' }}>
              Stocking Zimbabwe's most trusted building material brands. Wholesale and retail at prices that keep your project on budget.
            </motion.p>
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.2 }} className="flex flex-wrap gap-4 mt-8">
              <Link to="/contact" className="group inline-flex items-center gap-3 bg-orange-600 text-white px-8 py-4 text-sm uppercase tracking-[0.15em] font-semibold hover:bg-orange-700 transition-all" style={{ fontFamily: 'var(--font-sans)' }}>
                {hero.ctaPrimary} <ArrowRight size={16} className="group-hover:translate-x-1 transition-transform" />
              </Link>
              <Link to="/services" className="group inline-flex items-center gap-3 border border-gray-300 text-gray-600 px-8 py-4 text-sm uppercase tracking-[0.15em] font-semibold hover:border-orange-500 hover:text-orange-600 transition-all" style={{ fontFamily: 'var(--font-sans)' }}>
                {hero.ctaSecondary}
              </Link>
            </motion.div>
          </motion.div>
          <motion.div initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.9, delay: 0.2 }} className="relative">
            <div className="overflow-hidden aspect-[4/5]">
              <AnimatePresence mode="sync">
                <motion.img key={currentImg} src={heroImages[currentImg]} alt="Building materials"
                  className="absolute inset-0 w-full h-full object-cover object-center"
                  initial={{ opacity: 0, scale: 1.05 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }}
                  transition={{ duration: 1.5 }} loading="eager" />
              </AnimatePresence>
            </div>
            <div className="absolute -bottom-4 -right-4 bg-orange-600 text-white p-5 shadow-xl z-10">
              <Wrench size={24} weight="fill" className="mx-auto mb-1" />
              <div className="text-[9px] uppercase tracking-wider" style={{ fontFamily: 'var(--font-sans)' }}>Trade Ready</div>
            </div>
          </motion.div>
        </div>
      </div>
...
function BrandLogos() {
  const ref = useRef(null);
  const inView = useInView(ref, { once: true, margin: '-50px' });
  const brands = ['PPC Cement', 'Dulux', 'Proplastic', 'Safintra', 'First Brick', 'Dairibord'];
  return (
    <section ref={ref} className="bg-white py-10 border-y border-gray-100">
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        <motion.div initial={{ opacity: 0 }} animate={inView ? { opacity: 1 } : {}} transition={{ duration: 0.6 }}>
          <p className="text-center text-gray-400 text-[10px] uppercase tracking-[0.3em] mb-6" style={{ fontFamily: 'var(--font-sans)' }}>Trusted Brands We Stock</p>
          <div className="flex flex-wrap items-center justify-center gap-8 sm:gap-12">
            {brands.map((brand, i) => (
              <span key={i} className="text-gray-400 text-sm sm:text-base font-heading italic tracking-wider hover:text-orange-500 transition-colors cursor-default">{brand}</span>
            ))}
          </div>
        </motion.div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8 }} className="mb-14 sm:mb-20">
          ...
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-6">
            <div>
              <p className="text-orange-500 text-xs uppercase tracking-[0.3em] mb-3" style={{ fontFamily: 'var(--font-sans)' }}>Our Services</p>
            ...
            <Link to="/services" className="group text-gray-400 text-xs uppercase tracking-[0.2em] flex items-center gap-2 hover:text-orange-500 transition-colors" style={{ fontFamily: 'var(--font-sans)' }}>
              ...
              <motion.div key={service.title} initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}}
                ...
                <Link to={`/services#${services?.items?.[i]?.slug || ''}`}
                  ...
                  <div className="absolute top-4 right-5 z-10">
                    <span className="text-orange-500/15 font-heading text-6xl sm:text-7xl italic leading-none">{String(i + 1).padStart(2, '0')}</span>
                  </div>
                  <div className="absolute top-5 left-5 z-10 w-10 h-10 border border-orange-500/30 flex items-center justify-center bg-gray-900/60 backdrop-blur-sm group-hover:bg-orange-500/20 group-hover:border-orange-500/60 transition-all duration-500">
                    <IconComp size={18} weight="fill" className="text-orange-500" />
                  </div>
                  ...
                  <div className="absolute bottom-0 left-0 right-0 h-[2px] bg-gradient-to-r from-orange-500 to-orange-300 z-10" />
                </Link>
              </motion.div>
            );
          }}
    ...
    <section ref={containerRef} className="bg-white py-24 sm:py-32 lg:py-40 overflow-hidden">
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12 mb-12 sm:mb-16">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}}
          ...
          <div>
            <div className="w-12 h-[2px] bg-orange-500 mb-6" />
            <p className="text-orange-500 text-xs uppercase tracking-[0.3em] mb-3" style={{ fontFamily: 'var(--font-sans)' }}>Selected Works</p>
            <h2 className="font-heading text-gray-900 leading-[0.92] italic" style={{ fontSize: 'clamp(2.5rem, 6vw, 5rem)' }}>
              Our <span className="text-orange-500">Portfolio</span>
            </h2>
          </div>
          <Link to="/projects" className="group text-gray-400 text-xs uppercase tracking-[0.2em] flex items-center gap-2 hover:text-orange-500 transition-colors" style={{ fontFamily: 'var(--font-sans)' }}>
    ...
    <section ref={ref} className="bg-gray-50 py-24 sm:py-32">
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}}
          ...
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-6">
            <div>
              <p className="text-orange-500 text-xs uppercase tracking-[0.3em] mb-3" style={{ fontFamily: 'var(--font-sans)' }}>Recent Work</p>
              <h2 className="font-heading text-gray-900 leading-[0.92] italic" style={{ fontSize: 'clamp(2rem, 4.5vw, 3.5rem)' }}>
          ...
          {projects.items.slice(0, 6).map((project, i) => (
            <motion.div key={project.slug} initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}}
              ...
              <div className={`overflow-hidden relative ${i === 0 ? 'aspect-square sm:aspect-auto sm:h-full' : 'aspect-[4/3]'}`}>
                ...
                <div className="absolute inset-0 border-2 border-transparent group-hover:border-orange-500/30 transition-colors duration-500 z-10" />
                ...
                {/* Number watermark */}
                <div className="absolute top-4 right-5 z-10">
                  <span className="text-orange-500/10 font-heading text-5xl italic leading-none">{String(i + 1).padStart(2, '0')}</span>
                </div>
                ...
                {/* Bottom accent */}
                <div className="absolute bottom-0 left-0 right-0 h-[2px] bg-orange-500 z-10 scale-x-0 group-hover:scale-x-100 transition-transform duration-500 origin-left" />
              </div>
            </motion.div>
          ))}
        </div>
      ...
      <div className="animate-marquee flex whitespace-nowrap">
        {repeated.map((item, i) => (
          <span key={i} className="flex items-center gap-6 sm:gap-8 mx-6 sm:mx-8">
            <span className="text-orange-500 font-heading text-lg sm:text-2xl italic tracking-wider">{item}</span>
            <span className="text-orange-500/30 text-sm">&diams;</span>
          </span>
        ))}
      </div>
    </section>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-10 sm:gap-16">
          {stats.map((stat, i) => (
            <motion.div key={stat.label} initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}}
              ...
              <div className="font-heading text-orange-500 leading-none italic" style={{ fontSize: 'clamp(2.8rem, 6vw, 5rem)' }}>
...
function DepartmentCards() {
  ...
  const inView = useInView(ref, { once: true, margin: '-80px' });
  const depts = [
    { title: 'Building Materials', desc: 'Cement, bricks, sand, stone, aggregate, and blocks.' },
    { title: 'Roofing & Ceilings', desc: 'IBR sheets, corrugated iron, tiles, and ceiling boards.' },
    { title: 'Plumbing & Sanitary', desc: 'Pipes, fittings, valves, basins, and toilet sets.' },
    { title: 'Electrical & Solar', desc: 'Wiring, switches, lights, inverters, and panels.' },
  ];
  return (
    <section ref={ref} className="bg-white py-20 sm:py-28">
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          {depts.map((dept, i) => (
            <motion.div key={dept.title} initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}}
              transition={{ delay: i * 0.1 }} className="p-6 border border-gray-200 hover:border-orange-500/50 transition-colors group">
              <div className="flex items-start gap-4">
                <div className="w-10 h-10 bg-orange-500 flex items-center justify-center shrink-0">
                  <span className="text-white text-sm font-bold">{String(i + 1).padStart(2, '0')}</span>
                </div>
                <div>
                  <h3 className="text-gray-900 font-heading text-lg italic mb-1">{dept.title}</h3>
                  <p className="text-gray-500 text-sm leading-relaxed" style={{ fontFamily: 'var(--font-sans)' }}>{dept.desc}</p>
                </div>
              </div>
            </motion.div>
          ))}
      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        <div className="grid lg:grid-cols-2 gap-16 lg:gap-24 items-center">
          <motion.div initial={{ opacity: 0, x: -40 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.9 }}>
            <div className="w-12 h-[2px] bg-orange-500 mb-6" />
            <p className="text-orange-500 text-xs uppercase tracking-[0.3em] mb-3" style={{ fontFamily: 'var(--font-sans)' }}>Our Story</p>
            <h2 className="font-heading text-gray-900 leading-[0.95] italic mb-8" style={{ fontSize: 'clamp(2rem, 5vw, 3.5rem)' }}>
              The The<br /><span className="text-orange-500">Story</span>
            </h2>
            {about.story.map((p, i) => (
            ...
            <div className="flex gap-10 sm:gap-16">
              <div>
                <div className="text-orange-500 font-heading text-3xl sm:text-4xl italic leading-none">{business.established}</div>
                <div className="text-gray-400 text-[10px] uppercase tracking-[0.2em] mt-2" style={{ fontFamily: 'var(--font-sans)' }}>Founded</div>
              </div>
              <div>
                <div className="text-orange-500 font-heading text-3xl sm:text-4xl italic leading-none">{business.projectsCompleted}</div>
                <div className="text-gray-400 text-[10px] uppercase tracking-[0.2em] mt-2" style={{ fontFamily: 'var(--font-sans)' }}>Completed</div>
              </div>
              <div>
                <div className="text-orange-500 font-heading text-3xl sm:text-4xl italic leading-none">{business.yearsExperience}</div>
          ...
          <motion.div initial={{ opacity: 0, x: 40 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.9, delay: 0.2 }} className="relative">
            ...
            <div className="absolute -bottom-3 -right-3 w-16 h-16 border-b-2 border-r-2 border-orange-500/30" />
          </motion.div>
        </div>
      </div>
    </section>
      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        <div className="grid lg:grid-cols-2 gap-16 lg:gap-20 items-center">
          ...
          <motion.div initial={{ opacity: 0, x: 40 }} animate={inView ? { opacity: 1, x: 0 } : {}}
            ...
            <div className="w-12 h-[2px] bg-orange-500 mb-6" />
            <p className="text-orange-500 text-xs uppercase tracking-[0.3em] mb-3" style={{ fontFamily: 'var(--font-sans)' }}>The Difference</p>
            <h2 className="font-heading text-gray-900 leading-[0.95] italic mb-12" style={{ fontSize: 'clamp(2rem, 4.5vw, 3rem)' }}>
              Why Choose <span className="text-orange-500">The</span>
            </h2>
    ...
    <section ref={ref} className="bg-white py-24 sm:py-32">
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}}
          ...
          <p className="text-orange-500 text-xs uppercase tracking-[0.3em] mb-3" style={{ fontFamily: 'var(--font-sans)' }}>What Drives Us</p>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {about.values.map((value, i) => (
            <motion.div key={value.title} initial={{ opacity: 0, y: 25 }} animate={inView ? { opacity: 1, y: 0 } : {}}
              ...
              className="relative p-6 sm:p-8 border border-gray-200 hover:border-orange-500/40 transition-all duration-500 group">

              {/* Number watermark */}
              <span className="absolute top-4 right-4 text-orange-500/8 font-heading text-6xl italic leading-none">
                {String(i + 1).padStart(2, '0')}
              </span>

              {/* Icon badge */}
              <div className="w-12 h-12 bg-orange-500/10 flex items-center justify-center mb-5 group-hover:bg-orange-500/20 transition-colors duration-500">
                <CheckCircle size={22} weight="fill" className="text-orange-500" />
              </div>
            ...
            <motion.div key={active} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.6 }}>
              ...
              <div className="flex flex-col items-center gap-3">
                ...
                <div className="flex items-center gap-0.5 mt-1">
                  {[...Array(t.rating)].map((_, i) => <Star key={i} size={12} weight="fill" className="text-orange-500" />)}
                </div>
              </div>
            </motion.div>
          </AnimatePresence>
          <div className="flex items-center justify-center gap-6 mt-12">
            <button onClick={prev} className="w-10 h-10 border border-gray-200 flex items-center justify-center text-gray-400 hover:text-orange-500 hover:border-orange-500/30 transition-colors" aria-label="Previous">
              <CaretLeft size={16} />
            </button>
            <div className="flex gap-2">
              {homeTestimonials.map((_, i) => (
                <button key={i} onClick={() => setActive(i)} className={`h-[2px] transition-all duration-500 ${i === active ? 'w-10 bg-orange-500' : 'w-3 bg-gray-200'}`} aria-label={`Testimonial ${i + 1}`} />
              ))}
            </div>
            <button onClick={next} className="w-10 h-10 border border-gray-200 flex items-center justify-center text-gray-400 hover:text-orange-500 hover:border-orange-500/30 transition-colors" aria-label="Next">
              <CaretRight size={16} />
            </button>
          </div>
        </motion.div>
  ...
  return (
    <section ref={ref} className="relative py-28 sm:py-36 lg:py-48 overflow-hidden">
      <motion.div className="absolute inset-0" style={{ y: bgY }}>
        <img src={siteData.hero.backgroundImages[0]?.url} alt="The Builders Home" className="w-full h-[130%] object-cover object-center" loading="lazy" />
      ...
      <div className="relative z-20 max-w-5xl mx-auto px-5 sm:px-8 lg:px-12 text-center">
        <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-80px' }} transition={{ duration: 1 }}>
          ...
          <h2 className="font-heading text-white leading-[0.92] italic mb-8" style={{ fontSize: 'clamp(2.5rem, 8vw, 6rem)' }}>
            READY TO<br /><span className="text-orange-500">GET STARTED?</span>
          </h2>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <Link to="/contact" className="group relative inline-flex items-center gap-3 bg-orange-500 text-white px-8 py-4 text-sm uppercase tracking-[0.15em] font-semibold hover:shadow-xl transition-all" style={{ fontFamily: 'var(--font-sans)' }}>
          ...
          <div className="mt-16 pt-8 border-t border-white/5 flex flex-col sm:flex-row items-center justify-center gap-6 text-xs text-white/25 uppercase tracking-[0.15em]" style={{ fontFamily: 'var(--font-sans)' }}>
            <a href={`tel:${business.phoneRaw}`} className="flex items-center gap-2 hover:text-orange-500 transition-colors"><Phone size={14} /> {business.phone}</a>
